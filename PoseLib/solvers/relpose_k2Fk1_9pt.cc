...
int relpose_k2Fk1_9pt(const std::vector<Eigen::Vector3d> &x1, const std::vector<Eigen::Vector3d> &x2,
                      std::vector<ProjectiveImagePair> *models_) {
    models_->clear();
...
//        std::cout << "S: " << svd.singularValues().transpose() << std::endl;
        ...
        Camera cam1 = Camera("SIMPLE_DIVISION", std::vector<double>{1.0, 0.0, 0.0, k1[i]}, -1, -1);
        Camera cam2 = Camera("SIMPLE_DIVISION", std::vector<double>{1.0, 0.0, 0.0, k2[i]}, -1, -1);
        models_->push_back(ProjectiveImagePair(F, cam1, cam2));
    }
    return models_->size();
}
}