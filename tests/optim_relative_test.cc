...
void setup_scene_3v(int N, CameraPose &pose12, CameraPose &pose13, std::vector<Point2D> &x1, std::vector<Point2D> &x2,
                    std::vector<Point2D> &x3, Camera &cam) {

    CameraPose p1 = random_camera();
    CameraPose p2 = random_camera();
    CameraPose p3 = random_camera();
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        Eigen::Vector2d xi;
        cam.project(p1.apply(Xi), &xi);
        x1.push_back(xi);

        cam.project(p2.apply(Xi), &xi);
        x2.push_back(xi);

        cam.project(p3.apply(Xi), &xi);
        x3.push_back(xi);
    }

    Eigen::Matrix3d R12 = p2.R() * p1.R().transpose();
    Eigen::Vector3d t12 = p2.t - p2.R() * p1.R().transpose() * p1.t;

    Eigen::Matrix3d R13 = p3.R() * p1.R().transpose();
    Eigen::Vector3d t13 = p3.t - p3.R() * p1.R().transpose() * p1.t;

    pose12 = CameraPose(R12, t12);
    pose13 = CameraPose(R13, t13);
}
...
bool test_shared_camera_threeview_relative_pose_normal_acc() {
    const size_t N = 10;
    std::string camera_str = "0 SIMPLE_DIVISION 1 1 1.2 0.0 0.0 -0.5";
    Camera camera;
    camera.initialize_from_txt(camera_str);

    CameraPose pose12, pose13;
    std::vector<Eigen::Vector2d> x1, x2, x3;
    setup_scene_3v(N, pose12, pose13, x1, x2, x3, camera);
    ...
    std::vector<size_t> camera_refine_idx = image_triplet.camera.get_param_refinement_idx(opt);
    NormalAccumulator acc;
    ThreeViewSharedCameraRefiner refiner(x1, x2, x3, camera_refine_idx, pose13.t.norm(), 1.0);
    acc.initialize(refiner.num_params);

    // Check that residual is zero
    acc.reset_residual();
    refiner.compute_residual(acc, image_triplet);
    double residual = acc.get_residual();
    REQUIRE_SMALL(residual, 1e-6);

    // Check the gradient is zero
    acc.reset_jacobian();
    refiner.compute_jacobian(acc, image_triplet);
    REQUIRE_SMALL(acc.Jtr.norm(), 1e-6);

    return true;
}

bool test_shared_camera_threeview_relative_pose_jacobian() {
    const size_t N = 10;
    std::string camera_str = "SIMPLE_DIVISION 2560 1152 1265.772 1257.333 571.722 0.005963";
    Camera camera;
    camera.initialize_from_txt(camera_str);
    ...
    opt.refine_focal_length = true;
    std::vector<size_t> camera_refine_idx = image_triplet.camera.get_param_refinement_idx(opt);
    ThreeViewSharedCameraRefiner<UniformWeightVector, TestAccumulator> refiner(x1, x2, x3, camera_refine_idx,
                                                                               pose13.t.norm(), 1.0);

    const double delta = 1e-6;
    double jac_err = verify_jacobian<decltype(refiner), ImageTriplet>(refiner, image_triplet, delta);
    REQUIRE_SMALL(jac_err, 1e-6)

    // Test that compute_residual and compute_jacobian are compatible
    TestAccumulator acc;
    acc.reset_residual();
    double r1 = refiner.compute_residual(acc, image_triplet);
    acc.reset_jacobian();
    refiner.compute_jacobian(acc, image_triplet);
    double r2 = 0.0;
    for (int i = 0; i < acc.rs.size(); ++i) {
...
std::vector<Test> register_optim_relative_test() {
    return {TEST(test_relative_pose_normal_acc), TEST(test_relative_pose_jacobian),
            ...
            TEST(test_shared_camera_threeview_relative_pose_normal_acc),
            TEST(test_shared_camera_threeview_relative_pose_jacobian)};
}